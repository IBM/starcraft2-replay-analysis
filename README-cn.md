...
2. 将一个 Starcraft 录像文件加载到 IBM Cloud Object Storage 中。
3. 将对象加载到 Jupyer notebook 中
4. 将处理后的录像文件加载到 Cloudant 数据库中进行存储
...
注册 IBM [Watson Studio](http://dataplatform.ibm.com/)。注册 Watson Studio，这会在您的 IBM Cloud 帐户中创建两个服务：``Apache Spark`` 和 ``Object Storage``。
